...
- 领域验证和覆盖策略属于核心。
- 文件读写的具体驱动可位于 GUI adapter 或基础设施模块，但何时读取/写入、内容如何解释、冲突如何影响领域状态以及失败如何映射为稳定领域错误属于核心；不得在页面、托盘或其他 GUI 入口重复实现同一文件业务流程。
- 用户提示、确认状态、类型化 IPC 结构和界面错误映射属于 GUI adapter。
...
- 未经明确批准的覆盖策略或 GUI 操作，写入不得覆盖现有目标。
...
- 为类型化 IPC 输出、可见错误映射和取消状态提供 GUI adapter 黑盒测试。
