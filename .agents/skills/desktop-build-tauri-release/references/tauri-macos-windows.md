...
- 默认 `macosSigningSelection = disabled`、`macosSigningSource = not-requested`，使用 `--no-sign` 且不探测环境；只有来源为 `configured|requested|channel-required` 才启用。直接分发启用 Developer ID 时，签名、公证、ticket stapling、Gatekeeper 与最终验证必须作为一个完整阶段；不可保留“只签名未公证”或失败后回退 unsigned 的候选。启用系统通知的 macOS 候选必须签名。
- `verify-dmg-layout.sh` 只读挂载本次选择形成的最终 DMG，验证唯一 app、Applications 链接、背景、非空布局文件与包内本地 release notes 字节一致；签名启用时必须是已签名、公证并 stapled 的字节。
