...
- 当前 `productDefinitionRequired = true`。在产品规格获得批准前，只维护中性工程基线和 `docs/GUI_APP_PROFILE.md` 已确认的桌面能力，不自行发明业务功能、产品文案、数据来源、远程地址或成功标准。
...
- 窗口标题固定为 `LokiMetis v{version}`；所有用户可见版本在展示边界先移除已有 `v`/`V`，再添加且只添加一个小写 `v`。机器版本不带前缀。
- 首次启动或持久状态缺失、损坏、越界时，主窗口回退为 1440×900、最小 960×640 并居中；只恢复仍与当前显示器工作区相交的尺寸、位置和最大化状态。
- 固定 `/settings` 页面展示应用、版本、本地更新日志、语言和浅色/深色/跟随系统主题，以及已启用的通知和自启 Switch。`/about`、`/sponsor`、`/test` 和相应导航/资源必须缺席。
