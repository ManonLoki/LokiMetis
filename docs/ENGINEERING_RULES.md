...
- 固定 `/settings` 页面展示应用、版本、本地更新日志、语言和浅色/深色/跟随系统主题、已启用的通知和自启 Switch，并集中承载唯一 Agent 复选面板、扫描间隔与自动清理。Hooks 目录与写入表单只位于 `/monitor/settings` 且消费统一选择，不得复制 Agent 复选；看板内容页不保留设置子路由；`/about`、`/sponsor`、`/test` 和相应导航/资源必须缺席。
