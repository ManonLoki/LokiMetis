...
fn creation_prompt_uses_exact_user_and_bundled_skill_paths() {
    ...
    assert!(prompt.contains(
        &serde_json::to_string(&skill_root.join("SKILL.md").to_string_lossy()).expect("路径应编码")
    ));
...
fn legacy_install_without_consent_stops_before_host_side_effects() {
    tauri::async_runtime::block_on(async {
        ...
        let script_before =
            std::fs::read(directory.join("renderer-inject.js")).expect("应读取兼容皮肤脚本");

        let error = service
            .install(SkinHostKind::Codex, &reference, false, None, false, false)
            .await
        ...
        assert_eq!(
            std::fs::read(directory.join("renderer-inject.js")).expect("拒绝后原始脚本应保持不变"),
            script_before
        );
...
fn recovered_legacy_skin_requires_unique_catalog_identity() {
    ...
    let config =
        super::read_theme_css_config(&root.join("user/legacy-theme"), theme.appearance.as_ref())
            .expect("应构建主题运行 CSS");
...
fn restarted_endpoint_uses_proven_ready_scan_without_profile_probe_state() {
    ...
    let selected = restarted_instance_for_endpoint(vec![other, restarted], CdpEndpoint::new(9342))
...
async fn recovered_runtime_rejects_probe_from_previous_generation() {
    ...
    let service = SkinService::new(root.join("builtin"), root.join("user"));
    let descriptor = load_descriptor(&root.join("user/old-skin"), "old-skin", SkinSource::User)
        .expect("应读取旧皮肤测试描述");
