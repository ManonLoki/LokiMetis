impl SkinService {
    ...
    pub async fn install(
        ...
        let selected_instance = match instance_id {
            Some(id) => Some(resolve_codex_instance(id).await?),
            None => {
                if platform_codex_processes().await?.len() > 1 {
                    return Err(AppError::new(
                        "skin.codex_instance_selection_required",
                        "检测到多个 Codex 实例，请在LokiMetis中选择应用目标。",
                    ));
                }
                None
            }
        };
        let target_key = selected_instance
            .as_ref()
                ...
                Some((
                    InstallSkinResult::Installed {
                        status: SkinStatus::running(&loaded.descriptor, 0, compatibility)
                            .for_instance(target_key.clone().unwrap_or_default()),
                    },
                    endpoint,
                ))
            } else {
        ...
        if let Some((result, endpoint)) = already_installed {
            if let Some(endpoint) = endpoint {
                try_activate_codex_window(endpoint).await;
            }
            return Ok(result);
        }

        let (mut browser, handler_task, connection_source, endpoint) =
            connect_or_launch(&mut cancel, selected_instance.as_ref()).await?;
        let target_key = target_key.unwrap_or_else(|| format!("endpoint:{}", endpoint.port));
        ...
        let join = tokio::spawn(async move {
            watch_pages(browser, handler_task, payload, watched_skin, cancel_rx).await
        });
        let mut runtime = self.runtime.lock().await;
        ...
        runtime.last_target = Some(target_key);
        let result = InstallSkinResult::Installed {
            ...
            .for_instance(runtime.last_target.clone().unwrap_or_default()),
        };
        drop(runtime);
        try_activate_codex_window(endpoint).await;
        Ok(result)
    ...
    pub async fn uninstall(&self, instance_id: Option<&str>) -> Result<SkinStatus, AppError> {
        let _operation = self.operation.lock().await;
        let cleanup_endpoint = match instance_id {
            Some(id) => resolve_codex_instance(id)
                .await?
                .debug_port
                .map(CdpEndpoint::new),
            None => None,
        ...
        let task = {
            ...
            let target = instance_id
                .map(str::to_owned)
                .or_else(|| runtime.last_target.clone());
            let task = target
                .as_ref()
                .and_then(|target| runtime.instances.remove(target))
                .and_then(|instance| instance.task);
            runtime.last_target = runtime.instances.keys().next().cloned();
            task
        };
        let affected_pages = if task.is_some() {
            stop_watch_task(task).await?
        } else if let Some(endpoint) = cleanup_endpoint {
            remove_from_endpoint(endpoint).await?
        } else {
            remove_from_existing_endpoint().await?
        };
